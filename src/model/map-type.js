class MapType {
    constructor(
        ...
        totalEV,
        evFunction,
        shouldDisplayEVOnMap,
        secondarySliderIncrement,
        customMapEnabled,
        compareMapEnabled,
        shouldListProjectionsFirst,
        shouldAlwaysReorderOutlines,
        regionIDToName,
        regionsToHideOnDisable,
        mapSettingsLayout,
        createMapSources,
    ) {
        this.id = id;
        ...
        this.currentMapSourceID = getCookie(
            this.id + currentMapSourceSettingIDSuffix,
        );

        this.currentMapSettings = {};
        for (var settingNum in this.mapSettingsLayout) {
            var isGlobal = globalMapSettings.some(
                (setting) =>
                    setting.id == this.mapSettingsLayout[settingNum].id,
            );
            this.currentMapSettings[this.mapSettingsLayout[settingNum].id] =
                getCookie(
                    (isGlobal ? "" : this.id + "-") +
                        this.mapSettingsLayout[settingNum].id,
                ) || this.mapSettingsLayout[settingNum].defaultValue;
        }
        ...
        if (this.currentCycle == null || this.currentCycle.length == 0)
            this.currentCycle = mapCycles[0] ?? allYearsCycle;
    ...
    async loadSVG() {
        ...
        let svgPathID = svgPathString.includes("/")
            ? svgPathString.split("/").reverse()[0]
            : svgPathString;
    ...
    getMapSourceIDs(cycle = this.currentCycle) {
        if (cycle == allYearsCycle) {
            return Object.entries(this.mapSourceIDs)
                .sort((pair1, pair2) => {
            ...
            return this.mapSourceIDs[cycle].concat(
                this.mapSourceIDs[allYearsCycle],
            );
        } else {
            let sourceIDs = [
                ...this.mapSourceIDs[allYearsCycle].filter(
                    (sourceID) => sourceID != this.customSourceID,
                ),
    ...
    setMapCycle(newCycle) {
        ...
        setCookie(
            this.id + currentMapCycleSettingIDSuffix,
            newCycle != this.mapCycles[0] ? newCycle : "",
        );
    }
    ...
    getCompareMapSource() {
        return this.compareSourceID
            ? this.getMapSources(this)[this.compareSourceID]
            : null;
    }
    ...
    setMapSettings(currentMapSettings) {
        ...
        for (var settingID in this.currentMapSettings) {
            ...
            setCookie(
                (isGlobal ? "" : this.id + "-") + settingID,
                this.currentMapSettings[settingID],
            );

            if (isGlobal) {
                currentGlobalMapSettings[settingID] =
                    this.currentMapSettings[settingID];
            }
        }
    }
    ...
    getMapSettingLayout(settingID, selfArg) {
        var self = selfArg || this;
        return self.mapSettingsLayout.find(
            (setting) => setting.id == settingID,
        );
    }
...
var globalMapSettings = [
    {
        id: "flipStates",
        title: "🔺 Flip States",
        type: MapSettingType.optionCycle,
        options: [
            { id: "show", title: "Shown", value: true },
            { id: "hide", title: "Hidden", value: false },
        ],
        defaultValue: "hide",
        reloadType: MapSettingReloadType.display,
    },
    {
        id: "pieStyle",
        title: "📊 Pie Style",
    ...
    },
    {
        ...
        options: [
            ...
            { id: "serif", title: "Bree Serif", value: "BreeSerif-Regular" },
        ],
        defaultValue: "mdy",
        reloadType: MapSettingReloadType.custom,
        customReloadFunction: (value) => {
            $("#maptextcontainer").css("font-family", value);
            if (value == "BreeSerif-Regular") {
                $(".date-display")
                    .css("font-family", "BreeSerif-Mono")
                    .css("line-height", "100%");
            } else {
                $(".date-display")
                    .css("font-family", "")
...
for (var settingNum in globalMapSettings) {
    currentGlobalMapSettings[globalMapSettings[settingNum].id] =
        getCookie(globalMapSettings[settingNum].id) ||
        globalMapSettings[settingNum].defaultValue;
}
