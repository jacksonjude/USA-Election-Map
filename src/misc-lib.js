const browserName = bowser.getParser(navigator.userAgent).getResult()
    .browser.name;
...
function roundValue(valueToRound, decimalPlaceToRound) {
    return (
        Math.round(valueToRound * Math.pow(10, decimalPlaceToRound)) /
        Math.pow(10, decimalPlaceToRound)
    );
}

function roundValueToPlace(valueToRound, figuresToInclude) {
    var decimalPlaceToRound = Math.floor(
        -Math.log(valueToRound) / Math.log(10) + figuresToInclude,
    );
...
function addCommaFormatting(num) {
    ...
    for (let i = 0; i < rawNumString.length; i++) {
        ...
        if (
            i != rawNumString.length - 1 &&
            (rawNumString.length - 1 - i) % 3 == 0
        ) {
            numString += ",";
        }
    }
...
function getDateString(
    ...
    switch (
        overrideDateFormat ||
        currentMapType.getMapSettings()["dateFormat"]
    ) {
        case "dmy":
            dateString =
                (useZeroPadding
        ...
        case "ymd":
            dateString =
                ...
                (useZeroPadding
                    ...
                    : date.getMonth() + 1) +
                delimiter +
                (useZeroPadding ? zeroPadding(date.getDate()) : date.getDate());
            break;
...
function getDecadeFromDate(dateForDecade) {
    return (
        Math.floor(((dateForDecade || new Date()).getFullYear() - 1) / 10) * 10
    );
}
...
function isPointInDiv(divElement, e) {
    return (
        isPointInDivHorizontally(divElement, e) &&
        isPointInDivVertically(divElement, e)
    );
}
