...
function createMarginEditDropdownItems(shouldSetDefault) {
    ...
    for (var marginID in marginNames) {
        if (marginID == "tilt") {
            continue;
        } // Hardcoding tilt to be excluded
        $("#marginsDropdownContainer").append(
            "<div class='dropdown-separator'></div>",
        );
        $("#marginsDropdownContainer").append(
            "<a id='" +
                marginID +
                "-edit' onclick='toggleMarginEditing(\"" +
                marginID +
                "\", this)'>" +
                marginNames[marginID] +
                "<span style='float: right; font-family: \"BreeSerif-Mono\"'>" +
                marginValues[marginID] +
                "</span></a>",
        );
    }
...
function toggleMarginEditing(marginID, div) {
    if (editMarginID) {
        var marginValueToSet =
            parseFloat($("#" + editMarginID + "-text").val()) ||
            defaultMarginValues[editMarginID];
        marginValueToSet =
            Math.round(marginValueToSet * Math.pow(10, 1)) / Math.pow(10, 1);
        ...
        if (
            marginValueToSet <
            marginValues[marginIDArray[marginIDArray.indexOf(editMarginID) + 1]]
        ) {
            marginValueToSet =
                marginValues[
                    marginIDArray[marginIDArray.indexOf(editMarginID) + 1]
                ];
        }
        if (
            marginIDArray.indexOf(editMarginID) > 0 &&
            marginValueToSet >
                marginValues[
                    marginIDArray[marginIDArray.indexOf(editMarginID) - 1]
                ]
        ...
        if (
            shouldRefreshMap &&
            currentMapSource.getCustomDefaultMargins() == null
        ) {
            setCookie(marginsCookieName, JSON.stringify(marginValues));
        ...
        $("#" + editMarginID + "-edit").html(
            marginNames[editMarginID] +
                "<span style='float: right; font-family: \"BreeSerif-Mono\"'>" +
                marginValues[editMarginID] +
                "</span>",
        );
    }
    ...
    if (marginID) {
        $(div).html(
            marginNames[marginID] +
                "<input class='textInput' type='text' id='" +
                marginID +
                "-text' value='" +
                marginValues[marginID] +
                "'>",
...
function addResetMarginsRow() {
    let standardMargins =
        currentMapSource.getCustomDefaultMargins() ?? standardMarginValues;
    ...
    $("#marginsDropdownContainer").append(
        "<a id='reset-margins' style='padding-top: 14rem; min-height: 25rem; text-align: center;' onclick='resetMargins()'>Reset</a>",
    );
}

function resetMargins() {
    marginValues = cloneObject(
        currentMapSource.getCustomDefaultMargins() ?? standardMarginValues,
    );
