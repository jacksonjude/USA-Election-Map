...
async function addConstantMarginToMap(
    baseMarginToAdd,
    partyToShift,
    partyToTake,
    shouldShiftNPV = true,
    ...
    if (partyToShift.getID() == TossupParty.getID()) {
        return;
    }
    ...
    if (
        ...
        partyToShift == null ||
        partyToShift.getID() == TossupParty.getID()
    ) {
        return;
    }
    ...
    for (var regionID in displayRegionDataArray) {
        ...
        if (
            (regionID == nationalPopularVoteID && !shouldShiftNPV) ||
            regionID.endsWith(subregionSeparator + statePopularVoteDistrictID)
        ) {
            continue;
        }

        if (displayRegionDataArray[regionID].disabled) {
            continue;
        }
        ...
        if (regionData.partyVotesharePercentages) {
            ...
            let candidateDataToIncreaseMargin =
                regionData.partyVotesharePercentages.find(
                    (candidateData) =>
                        candidateData.partyID == partyToShift.getID(),
                );
            if (!candidateDataToIncreaseMargin) {
                candidateDataToIncreaseMargin = {
                    candidate: getRegionCandidateName(
                        partyToShift.getID(),
                        regionData,
                    ),
                    partyID: partyToShift.getID(),
                    voteshare: 0,
                };

                regionData.partyVotesharePercentages.push(
                    candidateDataToIncreaseMargin,
                );
            }

            if (partyToTake) {
                let candidateDataToDecreaseMargin =
                    regionData.partyVotesharePercentages.find(
                        (candidateData) =>
                            candidateData.partyID == partyToTake.getID(),
                    );
                if (candidateDataToDecreaseMargin) {
                    candidateDataToDecreaseMargin.voteshare = Math.max(
                        candidateDataToDecreaseMargin.voteshare - marginToAdd,
                        0,
                    );
                }
            } else {
                for (let candidateData of regionData.partyVotesharePercentages) {
                    if (
                        getRegionCandidateName(
                            candidateData.partyID,
                            regionData,
                            candidateData,
                        ) ==
                        getRegionCandidateName(
                            candidateDataToIncreaseMargin.partyID,
                            regionData,
                            candidateDataToIncreaseMargin,
                        )
                    ) {
                    ...
                    candidateData.voteshare -=
                        (marginToAdd * candidateData.voteshare) /
                        (100.0 - candidateDataToIncreaseMargin.voteshare);
    ...
    currentCustomMapSource.updateMapData(
        displayRegionDataArray,
        getCurrentDateOrToday(),
        false,
    );
...
function getTippingPointRegion() {
    ...
    if (
        Math.max.apply(null, Object.values(partyTotals)) < majorityEVCount
    ) // If candidate with most EVs is less than 1/2 +1 of total, return 0
    {
        return 0;
    }
    ...
    while (greatestEVCount >= majorityEVCount) {
        let nextClosestState = Object.values(displayRegionDataArray).reduce(
            (min, state) => {
                return !state.disabled &&
                    state.margin < min.margin &&
                    state.partyID == winnerPartyID &&
                    !checkedStates.includes(state.region) &&
                    (!state.partyVotesharePercentages ||
                        state.partyVotesharePercentages.length <= 1 ||
                        state.partyVotesharePercentages[1].partyID ==
                            runnerUpPartyID)
                    ? state
                    : min;
            },
        ...
        greatestEVCount -= currentMapType.getEV(
            getCurrentDecade(),
            nextClosestState.region,
            displayRegionDataArray[nextClosestState.region],
            currentMapSource.getShouldSetDisabledWorthToZero(),
            currentMapSource.isCustom(),
        );
...
function getCurrentTotalEV() {
    ...
    for (var regionID in displayRegionDataArray) {
        ...
        totalEV += currentMapType.getEV(
            getCurrentDecade(),
            regionID,
            displayRegionDataArray[regionID],
        );
    }
    return totalEV;
}
...
function toggleEnteringShiftAmount() {
    if (!isEnteringShiftAmount && !selectedParty) {
        return;
    }
    ...
    if (isEnteringShiftAmount) {
        $("#shift-other").html(
            "<span style='float: left'>Shift All by </span><span style='font-family: \"BreeSerif-Mono\"'><input class='textInput' type='text' id='shift-other-text' value='0.0'></span>",
        );
...
function getTippingPointShift() {
    ...
    return [
        shiftMargin,
        politicalParties[sortedParties[1]],
        politicalParties[sortedParties[0]],
    ];
}
...
function getNPVShift() {
    ...
    if (currentEditingMode == EditingMode.voteshare) {
        ...
        npvData.partyVotesharePercentages.sort(
            (cand1, cand2) => cand2.voteshare - cand1.voteshare,
        );
        npvRunnerUp =
            politicalParties[npvData.partyVotesharePercentages[1].partyID];
    } else {
        shiftMargin = npvData.margin;
        const sortedParties = getSortedPoliticalPartyIDs();
        npvRunnerUp =
            politicalParties[
                sortedParties[0] == npvLeader.getID()
                    ? sortedParties[1]
                    : sortedParties[0]
            ];
    }
...
function updateShiftDropdownText() {
    ...
    $("#shiftTextTippingPoint").html(
        `+${decimalPadding(roundValue(shiftTippingPoint[0], 1), 1)}`,
    );
    $("#shiftTextTippingPoint").css(
        "color",
        shiftTippingPoint[1].getMarginColors().likely,
    );

    let npvTippingPoint = getNPVShift();
    if (npvTippingPoint) {
        $("#shiftTextNPV").html(
            `+${decimalPadding(roundValue(npvTippingPoint[0], 1))}`,
        );
        $("#shiftTextNPV").css(
            "color",
            npvTippingPoint[1].getMarginColors().likely,
        );
    } else {
        $("#shiftTextNPV").html("+0");
